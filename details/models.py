...
# ==============================
# HOME PAGE SDG BADGES (ADMIN MANAGED)
# ==============================

class HomeSDG(models.Model):
    """
    A single Sustainable Development Goal badge on the Home page.

    Replaces the 17 hardcoded cards so the Extension Office can correct a
    label, hide a goal, or reorder the grid without a developer. The badge
    artwork stays a static file (``static/sdg/<code>.png``) keyed by ``code``;
    only the text and visibility are admin-editable.
    """
    ...
    def save(self, *args, **kwargs):
        if self.order is None or self.order == 0:
            last = HomeSDG.objects.aggregate(Max("order"))["order__max"]
            self.order = (last or 0) + 1
        super().save(*args, **kwargs)


# ==============================
# SERVICES PAGE SECTION COPY (ADMIN MANAGED)
# ==============================

class ServiceSectionCopy(models.Model):
    """
    Editable headings for the Services page's built-in sections.

    The sections themselves (workflow rings, process records, template
    library, office forms, proposal checklist, status lifecycle) are rendered
    from their own data, but their eyebrows, headings, and helper text used to
    be hardcoded in the template. One row per section, seeded by migration.

    Two placeholders stay dynamic and are substituted when the page renders:

    * ``{total}`` in the workflow footer — the live sum of the phase weights.
    * ``{count}`` in the checklist heading — the live number of wizard steps.
    """

    class Key(models.TextChoices):
        WORKFLOW = "workflow", "Workflow at a glance"
        PROCESS = "process", "Process flow"
        TEMPLATES = "templates", "Template library"
        FORMS = "forms", "Office forms"
        CHECKLIST = "checklist", "Proposal checklist"
        LIFECYCLE = "lifecycle", "Status lifecycle"

    key = models.SlugField(max_length=30, unique=True, choices=Key.choices)
    ...
    def __str__(self):
        return self.heading or self.get_key_display()

    @classmethod
    def get_for(cls, key):
        obj, _ = cls.objects.get_or_create(
            key=key,
            defaults={"heading": dict(cls.Key.choices).get(key, key.title())},
        )
        return obj

    @classmethod
    def as_map(cls):
        """All blocks keyed by section, for cheap template lookup."""
        return {row.key: row for row in cls.objects.all()}

    def rendered_heading(self, **values):
        """Heading with the live placeholders substituted (never raises)."""
        try:
            return (self.heading or "").format(**values)
        except (KeyError, ValueError):
            return self.heading

    def rendered_footer(self, **values):
        """Footer note with the live placeholders substituted (never raises)."""
        try:
            return (self.footer_note or "").format(**values)
        except (KeyError, ValueError):
            return self.footer_note
